...
class Generator:
    ...
    def generate(self,x,keep,is_training):
        with tf.variable_scope("GAN/Generator",reuse=False):
            ...
            #orig 512
            conv2=tf.nn.dropout(tf.layers.conv2d_transpose(n1,filters=256,kernel_size=(3,3),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv2"), keep, name="drop2")#14
            n2=tf.layers.batch_normalization(conv2,training=is_training,name="batch_norm2")

            #conv3=tf.nn.dropout(tf.layers.conv2d_transpose(n2,filters=32,kernel_size=(3,3),strides=(1,1),padding="SAME",activation=tf.nn.leaky_relu,name="dconv3"), keep, name="drop3")#28
            ...
            conv4=tf.nn.dropout(tf.layers.conv2d_transpose(n3,filters=32,kernel_size=(3,3),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv4"), keep, name="drop4")#28
            n4=tf.layers.batch_normalization(conv4,training=is_training,name="batch_norm4")

            conv5=tf.nn.dropout(tf.layers.conv2d_transpose(n4,filters=8,kernel_size=(3,3),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv5"), keep, name="drop5")#28
            n5=tf.layers.batch_normalization(conv5,training=is_training,name="batch_norm5")

            out=tf.layers.conv2d_transpose(n5,filters=1,kernel_size=(3,3),strides=(2,2),padding="SAME",activation=tf.nn.sigmoid,name="dconv6")#128x128
        return tf.image.resize_images(out,[28,28])

    def generate2(self,x,keep,is_training):
        with tf.variable_scope("GAN/Generator",reuse=False):
            i_flat=tf.nn.dropout(tf.layers.dense(x,units=49*8),keep,name="drop0")
            i1=tf.reshape(i_flat,[-1,7,7,8])

            conv1=tf.nn.dropout(tf.layers.conv2d_transpose(i1,filters=1024,kernel_size=(3,3),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv1"), keep, name="drop1")#7x7
            n1=tf.layers.batch_normalization(conv1,training=is_training,name="batch_norm1")

            conv2=tf.nn.dropout(tf.layers.conv2d_transpose(n1,filters=512,kernel_size=(3,3),strides=(1,1),padding="SAME",activation=tf.nn.leaky_relu,name="dconv2"), keep, name="drop2")#14
            n2=tf.layers.batch_normalization(conv2,training=is_training,name="batch_norm2")
