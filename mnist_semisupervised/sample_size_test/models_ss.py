...
class Generator:
    ...
    def generate(self,x,keep,is_training):
        with tf.variable_scope("GAN/Generator",reuse=False):
            ...
            conv4=tf.nn.dropout(tf.layers.conv2d_transpose(n3,filters=1,kernel_size=(3,3),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv4"), keep, name="drop4")#28
            n4=tf.layers.batch_normalization(conv4,training=is_training,name="batch_norm4")

            out=tf.layers.max_pooling2d(n4, pool_size=[3,3],strides=[2,2])
        return out
...
class Discriminator:
    ...
    def classify(self,x,keep,is_training,reuse=False):
        with tf.variable_scope("GAN/Discriminator",reuse=reuse):
            ...
            a2=tf.nn.dropout(tf.layers.conv2d(n1,filters=32,kernel_size=(3,3),strides=(1,1),padding="SAME",activation=tf.nn.leaky_relu,name="conv2"),keep,name="drop2")#28
        ...
        return y,y_prob,tf.reshape(n4,[-1,512*7*7])