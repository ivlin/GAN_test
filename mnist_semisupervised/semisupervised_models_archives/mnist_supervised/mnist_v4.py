    ...
    def __init__(self, weights=None):
        ...
        if weights==None:
            ...
            with tf.variable_scope("GAN/Generator"):
                self.layers.append({'w':tf.Variable(tf.random_normal([100,100],stddev=0.1),name="h1"),
                ...
                self.layers.append({'w':tf.Variable(tf.random_normal([512,1024],stddev=0.1),name="h4"),
                                ...
                                'activation':tf.nn.relu})
                self.layers.append({'w':tf.Variable(tf.random_normal([1024,1024],stddev=0.1),name="h6"),
                                'b':tf.Variable(tf.random_normal([1,1024],stddev=0.0),name="h6_bias"),
                                'activation':tf.nn.relu})
                self.layers.append({'w':tf.Variable(tf.random_normal([1024,1024],stddev=0.1),name="h7"),
                                'b':tf.Variable(tf.random_normal([1,1024],stddev=0.0),name="h7_bias"),
                                'activation':tf.nn.relu})
                self.layers.append({'w':tf.Variable(tf.random_normal([1024,2048],stddev=0.1),name="h5"),
                                ...
                                'activation':tf.nn.relu})
                #output
                self.layers.append({'w':tf.Variable(tf.random_normal([2048,784],stddev=0.1),name="out"),
...
def discriminator(x,reuse=False,weights=None):
    ...
    if weights==None:
        with tf.variable_scope("GAN/Discriminator",reuse=reuse):
            ...
            h1=tf.Variable(tf.random_normal([int(784/4*15),1],stddev=0.0000001),name="h1")
            h1_b=tf.Variable(tf.random_uniform([1,1],minval=-0.00,maxval=0.0),name="h1_bias")
...
def train_gan(args):
    ...
    disc_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_labels, logits=y))+\
    ...
    for _ in xrange(1000):
        print("Iteration ",_)
        #mod_y=tf.reshape(mod_y,[None])
        for d_to_g in xrange(5):
            batch_xs, batch_ys = mnist.train.next_batch(100)
            mod_y=[]
            for i in xrange(len(batch_ys)):
                if batch_ys[i]==4:
                    mod_y.append([1.0])
                else:
                    mod_y.append([0.0])
            mod_y=np.array(mod_y)
            d_loss,d_steps=sess.run([disc_loss, disc_train_step], feed_dict={x: batch_xs, y_labels: mod_y})
        for i in xrange(5):
    ...
    for it in xrange(5):
        ...
        output=sess.run([y], feed_dict={x: batch_xs, y_labels: mod_y})
