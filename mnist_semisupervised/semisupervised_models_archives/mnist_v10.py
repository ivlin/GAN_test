...
def train_gan(args):
    ...
    for _ in range(20):
      ...
      accuracy_l.append(accuracy.eval(session=sess,feed_dict={x: batch[0], 
                                                 y_labels: batch[1], 
                                                 keep_prob: 1.0,
                                                 is_training:False}))
    print(accuracy_l)
    ...
    for it in range(10):
        ...
        accuracies.append(sess.run([accuracy], feed_dict={x: xs, y_labels: ys, keep_prob: 1.0, is_training:False}))
    print(accuracies)
    print('test accuracy %g' % np.mean(accuracies))
    '''
