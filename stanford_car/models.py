...
class Generator:
    ...
    def generate(self,x,keep,is_training):
        with tf.variable_scope("GAN/Generator",reuse=False):
            ...
            conv2=tf.nn.dropout(tf.layers.conv2d_transpose(n1,filters=512,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv2"), keep, name="drop2")#32
            n2=tf.keras.layers.BatchNormalization(name="batch_norm2")(conv2,training=is_training)

            conv3=tf.nn.dropout(tf.layers.conv2d_transpose(n2,filters=32,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv3"), keep, name="drop3")#64
