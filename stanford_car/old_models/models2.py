...
class Generator:
    ...
    def generate(self,x,keep,is_training):
        with tf.variable_scope("GAN/Generator",reuse=False):
            ...
            conv1=tf.nn.dropout(tf.layers.conv2d_transpose(i1,filters=1024,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv1"), keep, name="drop1")#16
            n1=tf.keras.layers.BatchNormalization(name="batch_norm1")(conv1,training=is_training)

            conv2=tf.nn.dropout(tf.layers.conv2d_transpose(n1,filters=512,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="dconv2"), keep, name="drop2")#128
...
class Discriminator:
    ...
    def classify(self,x,keep,is_training,reuse=False):
        with tf.variable_scope("GAN/Discriminator",reuse=reuse):
            ...
            a1=tf.nn.dropout(tf.layers.conv2d(norm_x,filters=4,kernel_size=(5,5),strides=(1,1),padding="SAME",activation=tf.nn.leaky_relu,name="conv1"),keep,name="drop1")#28
            n1=tf.keras.layers.BatchNormalization(name="batch_norm1")(a1,training=is_training)

            a2=tf.nn.dropout(tf.layers.conv2d(n1,filters=32,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="conv2"),keep,name="drop2")#28
            n2=tf.keras.layers.BatchNormalization(name="batch_norm2")(a2,training=is_training)

            a4=tf.nn.dropout(tf.layers.conv2d(n2,filters=512,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="conv4"),keep,name="drop4")#14
            n4=tf.keras.layers.BatchNormalization(name="batch_norm4")(a4,training=is_training)

            a5=tf.nn.dropout(tf.layers.conv2d(n4,filters=1024,kernel_size=(5,5),strides=(2,2),padding="SAME",activation=tf.nn.leaky_relu,name="conv5"),keep,name="drop5")#14
